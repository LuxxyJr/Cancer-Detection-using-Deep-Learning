"""
LUNA16 Nodule Classification - Multi-Model 3-Fold Cross-Validation Pipeline

Uses pre-extracted .npy patches for fast training.
Run preextract.py FIRST to create the patch files.
...
Estimated time: ~8-15 hours on RTX 4050 (6GB VRAM)
"""
...
def read_manifest(patches_dir):
    ...
    if not os.path.exists(manifest_path):
        ...
        print(f"  Run preextract.py first to create patch files.")
        print(f"  Usage:  python preextract.py\n")
        return None
...
def balance_training_samples(samples, neg_ratio, seed):
    """Keep all positives, subsample negatives to exactly neg_ratio:1"""
    rng = random.Random(seed)
    pos = [s for s in samples if s["label"] == 1]
    neg = [s for s in samples if s["label"] == 0]
    n_neg = min(len(neg), len(pos) * neg_ratio)
    sampled_neg = rng.sample(neg, n_neg) if n_neg > 0 else []
    result = pos + sampled_neg
    rng.shuffle(result)
    return result
...
def compute_class_weights(samples):
    ...
    Formula: w_class = total / (num_classes * count_class)
    This gives higher weight to the minority class (nodules).

    Returns:
        list of [w_class0, w_class1]
    """
    n_total = len(samples)
    ...
    w0 = n_total / (num_classes * max(n_neg, 1))
    w1 = n_total / (num_classes * max(n_pos, 1))
...
def train_model_cv(model_config, manifest, folds, device, pipeline_start,
    ...
    Args:
        model_config: dict with 'name', 'arch', 'gradcam_layer'
        manifest: list of all sample dicts
        folds: list of K sets of patient UIDs
        device: "cuda" or "cpu"
        pipeline_start: time.time() when pipeline started
        all_model_results: dict to store results keyed by model name

    Returns:
        dict with all results for this model
    """
    ...
    for fold_idx in range(K_FOLDS):
        ...
        # Use ALL training samples (5:1 neg:pos from preextract).
        # WeightedRandomSampler ensures balanced mini-batches while
        # exposing the model to different negatives each epoch.
    ...
    ax.set_title(f"{model_name} ROC - {K_FOLDS}-Fold CV", fontsize=14)
...
def main():
    ...
    print("\n" + "=" * 80)
    print("  LUNA16 NODULE CLASSIFICATION")
    ...
    print(f"  Total patches:   {len(manifest)} "
          f"({n_pos} nodule, {n_neg} non-nodule)")
    ...
    for i, fold_uids in enumerate(folds):
        ...
        print(f"    Fold {i+1}: {len(fold_uids):3d} patients, "
              ...
              f"({fold_pos} nodule, {fold_neg} non-nodule)")
    ...
    ax.set_xlabel("False Positive Rate", fontsize=12)
    ax.set_ylabel("True Positive Rate (Sensitivity)", fontsize=12)
    ax.set_title(f"Model Comparison - Aggregate ROC ({K_FOLDS}-Fold CV)",
                 fontsize=14)
    ...
    with open(results_path, "w") as f:
        f.write(f"LUNA16 Nodule Classification - Multi-Model "
